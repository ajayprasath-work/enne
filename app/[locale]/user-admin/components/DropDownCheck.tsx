...
const DropDownCheck: React.FC<ButtonProps> = ({ label, onClick, disabled }) => {
    return (
        <button
            onClick={onClick}
            className={`flex bg-[#f3fbfc] items-center gap-4 rounded py-2.5 px-4 group min-w-[205px] border hover:text-white hover:bg-red-850 text-primary border-[#E7F7FA] ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={disabled}
        >
            <input
                type="checkbox"
